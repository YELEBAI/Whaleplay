...
import type { Preset, PresetItem, CreatePresetInput, UpdatePresetInput, CreatePresetItemInput, UpdatePresetItemInput } from '@neo-tavern/shared'
...
export const usePresetStore = create<PresetState>((set, get) => ({
  ...
  addItem: async (presetId, input) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        activePreset: state.activePreset?.id === presetId
  ...
  updateItem: async (presetId, itemId, input) => {
    ...
    try {
      ...
      const updateItems = (items: PresetItem[]) =>
  ...
  deleteItem: async (presetId, itemId) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        activePreset: state.activePreset?.id === presetId
          ? { ...state.activePreset, items: state.activePreset.items.filter((i) => i.id !== itemId), updatedAt: new Date().toISOString() }
          : state.activePreset,
      }))
    } catch (err) {
      set({ error: (err as Error).message })
